...
- **App Creator tooling**
  ...
  - **Initial App Creator Skills**
    ...
    2. **build-app - Build the App** — Selects the golden path; creates and modifies the app; produces tests, documentation, test/UAT environments, and deployment configuration. App Creator could spend weeks iterating with this skill. Each change is committed to the repo via PRs. This could also be multi-user building.
    3. **submit-app - Prepare the Change** — Reviews work, performs final checks, then submits a production deployment request to IT.
- **Strategies on how to apply the SDLC to the workflow of an App Creator.**
...
- **Likely touchpoints for security staff, platform engineering staff, and developer team staff.**
  ...
  - IT may choose to review a new project before prod deployment.
...
- **A maturity model for starting small and more human-driven, toward a mostly agent-driven lifecycle.**
  ...
  ![App Creator Framework maturity model: Level 1 Ad hoc experiments through Level 5 Self-serve production, with App Creator autonomy increasing and IT involvement decreasing.](docs/images/maturity-model.png)

## The App Creator Job Role

I think **AI App Creator, or just App Creator,** is the best fit. I hope it sticks; here's why:
...
**Why AI App Creator, or just App Creator?** - I like the word Creator rather than Developer if we're talking about a role these non-developer staff are taking on. Using AI rather than Agentic is deliberate to avoid too much technical jargon. They create apps with AI. **Keeping developer/coder out of the name ensures these app creators aren't confused with someone who speaks developer terminology or does traditional developer work.** It's also short, because casual conversation in an org tends to shorten formal roles to 1-2 words or acronyms. "I got approved to be an App Creator today" sounds like something good and even fun to do. **It's a creative act, and they are making apps, and this title isn't overloading an existing job title. It's unique and self-describing in its purpose, and doesn't sound like an IT job.** They're like "Content Creators", only their content is code.

Other names I've heard for this new job role that I disagree with:

**Corporate Vibe Coder** - Sadly, vibe coder has too much negative sentiment around it, and qualifying it with something like "Corporate Vibe Coder" will likely just get shortened back down to Vibe Coder around the office. In addition, this role doesn't fit the original definition of vibe coder: developers generating code they *could* read but choose not to. The ACF archetype is for non-developers who rely on AI agents and IT governance to build solutions for their business needs, largely through automation provided by development staff.
...
Before taking on the ACF for implementation, you should meet some requirements. Since the market around this solution is changing rapidly, and many tools are in their infancy, this isn't for everyone.
...
- Claude GitHub App - install for App Creator users and Org/repos they will work in. [https://github.com/apps/claude](https://github.com/apps/claude)
...
- Copilot via web. No local non-dev Copilot tool exists [https://github.com/copilot](https://github.com/copilot)
  - Copilot web automatically uses cloud environments (not the same as codespaces, which are developer-specific). This may differ from the new preview of cloud sandboxes, which is [a billable charge](https://docs.github.com/en/copilot/concepts/about-cloud-and-local-sandboxes), and may be limited to Copilot CLI/GUI for now.
- App Creators need to be contributors to the GitHub repos housing their projects.
  - Agents will do all GitHub interaction on their behalf, co-authoring commits.
- Skills need to be in-repo, as Copilot doesn't yet support centralized org skill marketplaces. Copilot *does* support centralized [custom-agent profiles](https://docs.github.com/en/copilot/how-tos/administer-copilot/manage-for-organization/prepare-for-custom-agents) that could fill part of that gap.
...
- In Notion, enable the Claude or Cursor [Custom Agent](https://www.notion.com/blog/introducing-custom-agents) for each App Creator. Connect it to the GitHub repos for their projects. Notion can have many Custom Agents, but Claude/Cursor are special external Agents. Claude uses [Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) but is hosted by Notion and is part of Notion AI credits ([more info](https://www.notion.com/help/guides/how-to-set-up-claude-agents-on-your-teams-notion-task-board#how-do-external-agents-work-in-notion)). The Cursor one is hosted by Cursor and requires a Cursor subscription. Both provide full development abilities via cloud sandboxes.
